# dashboard.py - VERSÃO COM LOGIN SIMPLES E AUTOCOMPLETE DE PORTOS
...
# -----------------------------------------------------------------------------
# 1. SEÇÃO DE AUTENTICAÇÃO (VERSÃO SIMPLES E CENTRALIZADA)
# -----------------------------------------------------------------------------
def check_password():
    ...
    # Layout para centralizar o formulário
...
def carregar_lista_de_portos():
    """Carrega uma lista de portos para o autocomplete. (Simulação)"""
    # No futuro, isso pode vir de uma API ou de um arquivo mais completo.
    # Por agora, vamos usar os portos do nosso arquivo mock.
    ...
    # Lista de fallback se o arquivo não for encontrado
...
with tab_monitoramento:
    ...
    if df_frota is None:
        ...
        with sub_tab_cards:
            # ... (código dos cards, sem alteração)
...
# -----------------------------------------------------------------------------
# ABA 2: EXPLORAÇÃO GLOBAL (COM AUTOCOMPLETE)
# -----------------------------------------------------------------------------
with tab_exploracao:
    ...
    porto_selecionado = st.selectbox(
        "Comece a digitar o nome de um porto:",
        options=lista_portos,
        index=None, # Nenhum porto selecionado por padrão
        placeholder="Selecione ou digite o nome do porto..."
    )

    if st.button("Buscar Navios no Porto", key="busca_porto_btn"):
        if not porto_selecionado:
            st.warning("Por favor, selecione um porto da lista.")
        else:
